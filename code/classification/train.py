...
# 生成热力图的函数
def generate_heatmap(attention_map, save_path):
    """
    生成并保存热力图
    """
    # 将 attention_map 从 GPU 转移到 CPU，并去除 batch 维度
    ...
    # 生成热力图
    plt.imshow(attention_map, cmap='jet', alpha=0.5)  # alpha 控制透明度
    plt.colorbar()  # 显示颜色条
...
def main(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, (x, y) in tqdm(enumerate(train_loader)):
            ...
            # 只为每个批次的第一张图片生成热力图
            #if i%2 == 0:
                #save_path = f"./heatmaps/epoch_{epoch}_batch_{i}_attention_map.png"
                # generate_heatmap(attention_map[0], save_path)  # 只对第一张图片生成热力图
